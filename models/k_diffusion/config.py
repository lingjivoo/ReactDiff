...
from . import layers, Diffusion, utils, augmentation
...
def load_config(file):
    defaults = {
        'model': {
            ...
            'mapping_cond_dim': 0,
            'unet_cond_dim': 0,
            'cross_visual_cond_dim': 58,
            # 'cross_visual_cond_dim': 0,
            'cross_audio_cond_dim': 768,
...
def make_model(config):
    ...
    model = Diffusion.ReactDiff(
        ...
        cross_visual_cond_dim=config['cross_visual_cond_dim'],
        cross_audio_cond_dim=config['cross_audio_cond_dim'],
...
def make_denoiser_wrapper(config):
    ...
    if loss_config == 'karras':
        ...
        weight_kinematics_loss = config.get('weight_kinematics_loss', 0.01)
        weight_velocity_loss = config.get('weight_velocity_loss', 1.0)
        if not has_variance:
            return partial(layers.Denoiser, sigma_data=sigma_data, weighting=weighting, weight_kinematics_loss= weight_kinematics_loss, weight_velocity_loss = weight_velocity_loss)
        return partial(layers.DenoiserWithVariance, sigma_data=sigma_data, weighting=weighting, weight_kinematics_loss= weight_kinematics_loss, weight_velocity_loss = weight_velocity_loss)
    if loss_config == 'simple':
        if has_variance:
            raise ValueError('Simple loss config does not support a variance output')
        return partial(layers.SimpleLossDenoiser, sigma_data=sigma_data, weight_kinematics_loss=weight_kinematics_loss, weight_velocity_loss=weight_velocity_loss)
